...
export async function getAllProducts(opts = {}) {
  ...
  if (opts.filter && opts.filter.inventory) {
    switch (opts.filter.inventory) {
      case 'discontinued':
        whereClause = 'WHERE p.discontinued = 1';
        break;
      case 'needs-reorder':
        whereClause =
          'WHERE p.discontinued = 0 AND (p.unitsinstock + p.unitsonorder) < p.reorderlevel';
        break;
    }
  }

  return await db.all(sql`
    SELECT ${ALL_PRODUCT_COLUMNS.map(columnName => `p.${columnName}`).join(',')},
